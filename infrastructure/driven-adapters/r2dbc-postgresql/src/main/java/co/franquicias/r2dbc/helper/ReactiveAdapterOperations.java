package co.franquicias.r2dbc.helper;
...
public abstract class ReactiveAdapterOperations<E, D, I, R extends ReactiveCrudRepository<D, I> & ReactiveQueryByExampleExecutor<D>> {
    ...
    public Mono<E> mergeNonNullAndSave(I id, E partialEntity) {
        return repository.findById(id)
                .switchIfEmpty(Mono.error(new IllegalArgumentException("Recurso no encontrado")))
                .flatMap(existingData -> {
                    D patchData = toData(partialEntity);
                    copyNonNullProperties(patchData, existingData);
    ...
    protected void copyNonNullProperties(D source, D target) {
        try {
            ...
            for (java.lang.reflect.Field field : fields) {
                ...
                if (value != null && !field.getName().equals("id") && !field.getName().equals("version") && !field.getName().equals("createdAt")) {
